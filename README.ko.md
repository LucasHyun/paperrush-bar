...
| **즐겨찾기** | 실제로 노리는 학회에 ★ 를 찍으면 메뉴바가 그 학회들만 따릅니다. 알림도 같게 맞출 수 있습니다. 별표는 연도가 아니라 학회에 붙으므로 다음 회차로 그대로 이어집니다 |
...
(월요일 06:30 UTC, 원본 작업 직후) 각 마감의 `sourceUrl` 을 다시 읽고 **Gemini** (Flash-Lite 모델, 구글이 모델을 내리면 더 새로운 모델로 자동 전환) 로
