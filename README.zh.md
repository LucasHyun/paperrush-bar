...
| | |
|---|---|
| **菜单栏倒计时** | 在你收藏的会议之间轮流显示最近的投稿截止（形如 `ICLR D-5`），进入 D-3 后固定在该会议；沙漏中的沙量表示 30 天内剩余的时间，并从 D-7 起由黄 → 橙 → 红渐变，跨过午夜时会落下一粒沙，进入 D-3 后沙会自行流动，越接近截止越快，D-1 起还会不安地轻轻晃动（遵循“减弱动态效果”设置） |
...
| **收藏** | 给真正要投的会议加 ★，菜单栏便只跟随这些会议；通知也可同样设置。收藏属于会议本身而非某一年，会自动延续到下一届 |
| **点击打开官网** | 点击任意一行即可打开该会议官方网站 |
| **补充会议** | 18 个上游未收录 + 7 个只收录一半的会议，每周从各自 CFP 重新读取更新，也可添加你自己的会议 |
| **自己核对** | 在齿轮菜单中填入你自己的 Gemini API 密钥，应用会读取各会议官网，只提议在所引用页面上确实出现的日期。未经确认不会改动任何数据 |
| **登录时自动启动** | 通过 `SMAppService` 一键开关 |
| **三种语言** | English · 한국어 · 中文，齿轮菜单中即时切换（默认跟随系统语言） |
| **新版本通知** | 有新版本时通知一次 —— 每个版本只提醒一次，并有独立开关 |
...
重新读取每条截止的 `sourceUrl`，用 **Gemini**（Flash-Lite 模型；若 Google 下线该模型会自动换用更新的模型）提取日程，只提交能够验证的结果。CFP 一旦公布，
